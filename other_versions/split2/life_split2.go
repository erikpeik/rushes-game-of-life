...
func main() {
	...
	if len(os.Args) != 3 {
		println("Usage: ./life_split2 initial_state iterations")
		os.Exit(1)
	}
	...
	for g := 0; g < iterations; g++ {
		has_changed = false
		wg.Add(2)
		part_check(0, 0, height, width/2, width, height, array)
		part_check(0, width/2, height, width, width, height, array)
		wg.Wait()
